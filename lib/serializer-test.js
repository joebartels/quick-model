  ...
  describe('instance', function() {
    describe('#normalize', function() {
      describe('#normalizeAttribute', function() {
        describe('include specific attribute', function() {
          const rawData = {
            name: 'Amanda Seales',
            age: 36
          };

          const comedian = new Model({
            name: attr(mockTransform()),
            age: attr(mockTransform())
          });

          const normalizer = new Serializer({ model: comedian });

          it('only normalizes those from `include`', function() {
            expect(normalizer.normalize(rawData, { include: ['name'] }))
              .to.deep.equal({ name: 'Amanda Seales'});
          });
        });

        describe('exclude specific attribute', function() {
          const rawData = {
            name: 'Amanda Seales',
            age: 36
          };
          ...
          it('does not normalize those from `exclude`', function() {
            expect(normalizer.normalize(rawData, { exclude: ['name'] }))
              .to.deep.equal({ age: 36});
          });
        });

        describe('include and exclude specific attribute', function() {
          const rawData = {
            name: 'Amanda Seales',
            age: 36
          };
          ...
          it('does not normalize those from `exclude`', function() {
            expect(normalizer.normalize(rawData, { include: ['age', 'name'], exclude: ['name'] }))
              .to.deep.equal({ age: 36});
          });
        });

        describe('no rawData provided', function() {
          const author = new Model({
            fullName: attr(mockTransform())
          });
          const publisher = new Model({
            company: attr(mockTransform())
          });
          const post = new Model({
            id: primaryKey(mockTransform()),
            title: attr(mockTransform()),
            author: one(author),
            publishers: many(publisher)
          });

          const serializer = new Serializer({ model: post });

          it('normalizes a posts\'s attributes', function() {
            expect(serializer.normalize({})).to.deep.equal({
              id: null,
              title: null,
              author: null,
              publishers: null
        ...
        describe('some rawData provided', function() {
          const author = new Model({
            fullName: attr(mockTransform())
          });
          const publisher = new Model({
          ...
          const rawData = {
            ...
            author: { fullName: 'Seth Stephens-Davidowitz' }
          };

          it('normalizes a post', function() {
            expect(serializer.normalize(rawData)).to.deep.equal({
              id: rawData.id,
              title: rawData.title,
      ...
      describe('#normalizeRelationship', function() {
        describe('exclude specified relationships', function() {
          const rawData = {
          ...
          it('excludes certain relationship', function() {
            expect(serializer.normalize(rawData, { exclude: ['posts'] }))
              .to.deep.equal({ author: rawData.author });
          });
        });

        describe('include specified relationships', function() {
          const rawData = {
            author: { fullName: 'bob ross' },
            posts: [
              { id: 1, title: 'post 1' },
          ...
          it('includes and excludes certain relationship', function() {
            expect(serializer.normalize(rawData, { include: ['posts'] }))
              .to.deep.equal({ posts: rawData.posts });
          });
        });

        describe('include and exclude specified relationships', function() {
          const rawData = {
            author: { fullName: 'bob ross' },
            posts: [
              { id: 1, title: 'post 1' },
          ...
          it('excludes certain relationship', function() {
            expect(serializer.normalize(rawData, { exclude: ['author'], include: ['posts'] }))
              .to.deep.equal({ posts: rawData.posts });
          });
        });
    ...
    describe('#serialize', function() {
      describe('#serializeRelationship', function() {
        describe('including specified relationships', function() {
          /**
            `serializer.serialize(data, { include: ['posts'] })`
          ...
          */
          ...
          it('serializes the "only" specified relationship', function() {
            expect(serializer.serialize(rawData, { include: ['posts'] }))
              .to.deep.equal({ posts: rawData.posts });
          });
        });
      ...
      describe('#serializeAttribute', function() {
        describe('using "only" specified attributes', function() {
          /**
            `serializer.serialize(data, { include: ['fullName'] })`
            should only serialize specified attributes/relationships
            - returning only the serialized `fullName`, and `birthYear` in this example:
          */
          const rawData = {
            fullName: 'Roxane Gay',
            birthYear: 1974,
          ...
          it('serializes the "only" specified attributes', function() {
            expect(serializer.serialize(rawData, { include: ['fullName', 'birthYear'] }))
              .to.deep.equal({
    ...
    describe('#deserialize', function() {
      describe('#deserializeRelationship', function() {
        describe('using "only" specified relationships', function() {
          /**
          `serializer.deserialize(data, { include: ['posts'] })`
          should only deserialize specified attributes/relationships
          - returning only the deserialized `posts` in this example:
          */
          const rawData = {
            author: { fullName: 'bob ross' },
            posts: [
          ...
          it('deserializes "only" specified relationship', function() {
            expect(serializer.deserialize(rawData, { include: ['posts'] }))
            .to.deep.equal({ posts: rawData.posts });
          });
        });
        describe('with embedded relationships', function() {
      ...
      describe('#deserializeAttribute', function() {
        describe('using "only" specified attributes', function() {
          /**
            `serializer.deserialize(data, { include: ['fullName'] })`
            should only deserialize specified attributes/relationships
            - returning only the deserialized `fullName` and `birthYear` in this example:
          */
          const rawData = {
            fullName: 'Roxane Gay',
            birthYear: 1974,
          ...
          it('deserializes "only" specified attributes', function() {
            expect(serializer.deserialize(rawData, { include: ['fullName', 'birthYear'] }))
              .to.deep.equal({
                fullName: rawData.fullName,
                birthYear: rawData.birthYear
              });
